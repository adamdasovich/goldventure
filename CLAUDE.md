...
| 2026-02-09 | Treasure Oakes Resources Inc. (ID 409) had wrong name and 0 news items | TWO issues: (1) Company name scraped as "Explore Northern Ontario's Rich Minerals" (tagline) instead of actual name. (2) Sitekit CMS with React hydration stores news data in JSON props embedded in HTML, NOT in standard DOM elements. Pattern: `"SectionCaption":"January 28, 2026"` (date), `"Link":"https://www.accessnewswire.com/..."` (URL), `"HtmlText":"<p class=\"plain\">..."` (title in escaped HTML). Detection: `SK.React.Component` in HTML or `nccdn.net` CDN. Fix: Added SITEKIT-JSON extraction strategy using regex to parse JSON properties and unescape HTML for title extraction. LESSON: Modern CMS platforms may embed news data in React hydration JSON rather than semantic HTML - always check raw HTML source for JSON patterns when standard DOM selectors fail. |
