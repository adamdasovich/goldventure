...
| 2026-02-09 | Tectonic Metals (ID 407) had 0 news items (should have ~40) | Site uses `/news/news-releases/` URL pattern (nested `/news/` section before `/news-releases/`). Existing patterns had `/news-releases/` and `/news/news-releases/{year}/` with year suffixes, but NOT the base `/news/news-releases/` path without a year. Tectonic shows all 40+ releases on the main page without pagination. Fix: Added `/news/news-releases/` pattern to BOTH `website_crawler.py` and `company_scraper.py`. LESSON: Always add BOTH the base path AND year-based variants for news URL patterns - some sites show all items on base path, others use year archives. |
