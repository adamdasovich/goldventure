...
- **WIX-RICHTEXT** strategy for Greenheart Gold (ID 340) - Wix sites with paired `wixui-rich-text__text` links (same href, one date, one title)
...
| 2026-02-05 | Golden Pursuit (ID 338) exchange not extracted | Website shows `GDP (TSX-V):` format. Ticker patterns had `EXCHANGE: TICKER`, `TICKER: EXCHANGE`, `(EXCHANGE: TICKER)` but NOT `TICKER (EXCHANGE)` format. Fix: Added `TICKER (EXCHANGE)` patterns to both `_extract_ticker_from_page()` and homepage ticker extraction. LESSON: Always check ALL ticker format variations when adding companies - `TICKER (EXCHANGE)` is a common format alongside `EXCHANGE: TICKER`. |
| 2026-02-05 | Golden Pursuit (ID 338) shares_outstanding parsed as 42.7 trillion | Regex `(million|M)?` captured `M` from "Market" in text "42,781,800 Market Cap". The `\s*` ate the space, then `(M)?` matched the first letter of "Market", causing 1,000,000x multiplication. Same bug existed for market_cap regex with `B` matching "Book". Fix: Use negative lookahead `M(?![a-zA-Z])` and `B(?![a-zA-Z])` to ensure M/B are only matched as standalone abbreviations, not as the start of another word. LESSON: When regex uses single-letter optional groups like `(M)?` or `(B)?`, always add negative lookahead `(?![a-zA-Z])` to prevent matching the first letter of the next word. |
| 2026-02-05 | Greenheart Gold (ID 340) had 0 news items | Wix site where each news item has TWO `<a class="wixui-rich-text__text">` links with the SAME href - one containing date (MM/DD/YYYY in `<h2>`), one containing title (in `<p>`). Links go to internal pages (`/YYMMDD-pr`) or PDFs (`/_files/ugd/...`). WIX-HTML requires `/news/slug` URL pattern (doesn't match), WIX-BUTTON requires `wixui-button` class (doesn't match). Fix: Added WIX-RICHTEXT strategy that groups `wixui-rich-text__text` links by href, identifies date vs title text, parses MM/DD/YYYY directly. LESSON: Wix has at least THREE news layout patterns: (1) WIX-HTML with `/news/slug` blog links, (2) WIX-BUTTON with external "Read More" buttons, (3) WIX-RICHTEXT with paired rich-text links to internal pages or PDFs. |
| 2026-02-05 | Homeland Uranium (ID 344) had 5 wrong dates (DD.MM.YYYY parsed as MM.DD.YYYY) | Joomla site uses DD.MM.YYYY date format (European). `parse_date_standalone()` defaults to MM.DD.YYYY for ambiguous dot-separated dates where both parts ≤ 12 (e.g., `08.01.2026` → August 1 instead of January 8). Fix: Added DD.MM.YYYY format detection in JOOMLA-CATLIST strategy - collects all dates on the page first, if ANY date has first number > 12 (e.g., `22.01.2026`), the format is DD.MM.YYYY, then all dates on that page are parsed as DD.MM.YYYY. LESSON: When parsing ambiguous date formats (DD.MM vs MM.DD), use context from SIBLING dates on the same page to detect the format. If any date has an unambiguous first number > 12, the whole page uses DD.MM format. |
| 2026-02-05 | Kootenay Silver (ID 350) had only 4 news items (should have ~39) | UIKit-based site uses ordinal date suffixes ("January 22nd, 2026", "January 8th, 2026"). `parse_date_standalone()` returned `None` for ALL ordinal dates because regex patterns like `\d{1,2}` don't match "22nd". The UIKIT strategy (line 4920) correctly identified the page structure and extracted items, but every item was skipped because date parsing failed. Fix: Added `re.sub(r'(\d)(st|nd|rd|th)\b', r'\1', text)` at the start of `parse_date_standalone()` to strip ordinal suffixes before any parsing. LESSON: When `parse_date_standalone()` returns `None` for dates that look valid, check for ordinal suffixes (st/nd/rd/th) - they break ALL downstream regex patterns. Strip them early. |
| 2026-02-05 | Metals Group (ID 358) had only 9 news items (should have ~38) | WordPress Divi site uses **Super RSS Reader Pro** plugin to display news from multiple subsidiary companies (Torr Metals, Emperor Metals, Altiplano, etc.) in tabbed grid. Plugin renders `div.srr-item` containers with `div.srr-title > a` for links and `time.srr-date` for dates. No existing strategy matched this structure. Items are server-side rendered (no JS needed). Links point to external domains (newsfilecorp.com, finance.yahoo.com, metaleromining.com). Fix: Added SRR extraction strategy. NOTE: metalsgroup.com blocks DigitalOcean IPs (curl times out), but crawl4ai browser cache from onboarding provides the HTML. LESSON: WordPress plugins like Super RSS Reader Pro create unique HTML structures that don't match standard WP patterns - need dedicated strategies. |
| 2026-02-09 | Starcore International Mines (ID 404) had 1 news item (should have ~60) | Multilingual site uses `/en/investors/news/YYYY/` URL pattern (combining `/en/` prefix + `/investors/` section + `/news/YYYY/` year archives). Existing patterns had `/en/news/YYYY/` and `/investors/news/YYYY/` separately but NOT the combined `/en/investors/news/YYYY/`. Also: company name was scraped as "Gold and Silver Producer" (tagline) instead of "Starcore International Mines". Fix: Added `/en/investors/`, `/en/investors/news/`, and `/en/investors/news/{year}/` patterns to BOTH `website_crawler.py` and `company_scraper.py`. LESSON: Multilingual sites often combine `/en/` or `/es/` prefix with section paths like `/investors/news/` - need to add these combined patterns, not just individual components. |
| 2026-02-09 | Tectonic Metals (ID 407) had 0 news items (should have ~40) | TWO issues: (1) URL pattern `/news/news-releases/` missing (only had year variants), (2) HTML extraction strategy missing for Blender CMS `div.news-listing > div.item` with `div.date`, `div.title`, `a.button` structure. Fix: Added `/news/news-releases/` URL pattern AND new NEWS-LISTING extraction strategy (for `div.item` with `.date`/`.title`/`a.button`). Also added `.news-listing .item` to company_scraper.py selectors. LESSON: New sites often need BOTH new URL patterns AND new extraction strategies - always check the HTML structure when URL patterns work but extraction fails. |
| 2026-02-09 | Transition Metals (ID 408) had 0 news items | Joomla site using Gridbox page builder with `.ba-blog-post` containers, `.ba-blog-post-title a` for title links, `.ba-blog-post-date span` for dates (ordinal format). Year-based URL patterns: `/{year}` for recent (2026, 2025) and `/{year}-press-releases` for older (2024 and earlier). Fix: Added BA-BLOG-POST extraction strategy and year-based URL patterns `/{year}` and `/{year}-press-releases`. LESSON: Joomla sites using Gridbox page builder have unique `ba-blog-*` CSS classes. Check for year-based URL patterns that may differ between recent and older archives. |
| 2026-02-09 | Treasure Oakes Resources Inc. (ID 409) had wrong name and 0 news items | TWO issues: (1) Company name scraped as "Explore Northern Ontario's Rich Minerals" (tagline) instead of actual name. (2) Sitekit CMS renders news in collapsible sections with `.secHead > .secTitle > a.secCptn[href] > span.fmCptnMain` (date), link href (URL), and `.secBody p.plain font.plainlarge` (title). Detection: `nccdn.net` CDN in HTML or `SK.React.Component`. Initial JSON regex approach failed because HtmlText in JSON doesn't contain the title - title is in rendered DOM. Fix: Added SITEKIT-DOM extraction strategy that parses the rendered HTML structure. LESSON: When CMS uses React hydration, the JSON props may only contain metadata (dates, links) while actual content (titles) is in the rendered DOM - try DOM selectors first before JSON parsing. |
| 2026-02-09 | Western Exploration (ID 424) had only 1 news item (should have 4) | Q4 Web Systems CMS (q4cdn.com CDN) uses URL pattern `/news-and-updates/news/default.aspx` with news detail URLs ending in `/default.aspx`. THREE issues: (1) URL pattern missing - added `/news-and-updates/news/default.aspx` and year variants. (2) HTML extraction strategy missing - added MODULE-NEWS strategy for `.module_item` containers with `.module_date-text` and `.module_headline-link`. (3) CRITICAL: Slug deduplication bug - `extract_url_slug()` returned `default.aspx` for ALL news URLs, causing `_add_news_item()` slug deduplication AND tertiary deduplication at end of function to filter out 3 of 4 items. Fix: Added `generic_slugs = {'default.aspx', 'index.html', 'index.php', ...}` exclusion list to BOTH deduplication locations. LESSON: When all news URLs end with the same filename (default.aspx, index.html), slug-based deduplication incorrectly treats them as duplicates. Exclude common page filenames from slug matching. |
