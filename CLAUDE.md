...
| 2026-02-04 | Prospector Metals (ID 305) had 0 news and no presentation | TWO issues: (1) FLEX-WRAP strategy in `website_crawler.py` wasn't targeting the right containers (needed `-mx-` class filter for Tailwind layout containers vs navbars). (2) `_scrape_document_subpage()` in `company_scraper.py` only extracted `<a href=".pdf">` links but NOT iframe PDFs. Many ProcessWire/Adnet sites use ViewerJS iframes: `<iframe src="/viewerjs/#../path/to/file.pdf">`. Fix: Added iframe PDF extraction to `_scrape_document_subpage()` and added code in `views.py` to set `Company.presentation` from first presentation document found. LESSON: When documents are embedded in iframes (not direct links), scraper must parse iframe `src` attributes. |
