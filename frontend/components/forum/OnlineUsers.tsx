...
export function OnlineUsers({ users, typingUsers }: OnlineUsersProps) {
  ...
  // Capitalising the raw value rendered `mining_company` as "Mining_company"
  // on every badge. Values no longer offered at registration still appear here:
  // `company` is written by the access-request approval, and older accounts
  // hold `student` and `analyst`.
  ...
  const getUserTypeLabel = (userType?: string) => {
    ...
    if (USER_TYPE_LABELS[userType]) return USER_TYPE_LABELS[userType];
    // Anything unmapped still reads as words rather than a database value.
    return userType
      .split("_")
            ...
            {users.map((user) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {isTyping && (
                      <div className="flex items-center gap-1 mt-1">
                        ...
                        <span className="text-xs text-gold-400 italic">
                          typing...
                        </span>
                      </div>
                    )}
                  ...
                  {/* User type badge */}
                  <Badge
                    variant={getUserTypeVariant(user.user_type)}
                    className="text-xs"
                  >
