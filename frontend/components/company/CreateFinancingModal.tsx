...
interface CreateFinancingModalProps {
  ...
  onClose: () => void;
  onCreateComplete: (financingId?: number) => void;
}
...
const FINANCING_TYPES = [
  {
    ...
    description: "Non-public offering to select investors",
  },
  {
    value: "bought_deal",
    label: "Bought Deal",
    description: "Underwriter commits to full financing",
  },
  {
    value: "flow_through",
    label: "Flow-Through Shares",
    description: "Tax-advantaged Canadian exploration financing",
  },
  {
    value: "rights_offering",
    label: "Rights Offering",
    description: "Existing shareholders can purchase new shares",
  },
  {
    value: "warrant_exercise",
    label: "Warrant Exercise",
    description: "Existing warrant holders exercise their warrants",
  },
  {
    value: "debt",
    label: "Debt Financing",
    description: "Loan or credit facility",
  },
  {
    value: "royalty_stream",
    label: "Royalty/Stream",
    description: "Upfront payment for future production royalties",
  },
...
const UNIT_TYPES = [
  {
    value: "shares",
    label: "Common Shares Only",
    description: "Simple equity offering",
  },
  {
    value: "units",
    label: "Units (Shares + Warrants)",
    description: "Each unit includes shares and warrants",
  },
  {
    value: "flow_through_shares",
    label: "Flow-Through Shares",
    description: "Tax-advantaged shares for Canadian investors",
  },
];

export function CreateFinancingModal({
  companyId,
  companyName,
  accessToken,
  onClose,
  onCreateComplete,
}: CreateFinancingModalProps) {
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        warrant_strike_price:
          hasWarrants && warrantStrikePrice
            ? parseFloat(warrantStrikePrice)
            : null,
        warrant_expiry_date:
          hasWarrants && warrantExpiryDate ? warrantExpiryDate : null,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || errorData.detail || "Failed to create financing",
        );
      }

      const created = await response.json();
      onCreateComplete(created?.id);
      onClose();
    } catch (err) {
      console.error("Failed to create financing:", err);
      setError(
        err instanceof Error ? err.message : "Failed to create financing",
      );
    } finally {
      setCreating(false);
    }
        ...
        <Card className="relative z-10 w-full max-w-3xl p-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-center gap-2 mb-6">
            {[1, 2, 3].map((s) => (
              <div key={s} className="flex items-center">
                ...
                {s < 3 && (
                  <div
                    className={`w-12 h-1 ${step > s ? "bg-gold-500" : "bg-slate-700"}`}
                  />
                )}
              </div>
            ))}
          ...
          {step === 1 && (
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {FINANCING_TYPES.map((type) => (
                    ...
                    >
                      <p
                        className={`font-medium ${financingType === type.value ? "text-gold-400" : "text-white"}`}
                      >
                        {type.label}
                      </p>
              ...
              {(financingType === "private_placement" ||
                financingType === "bought_deal") && (
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-3">
                    ...
                    {UNIT_TYPES.map((type) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            <p
                              className={`font-medium ${unitType === type.value ? "text-gold-400" : "text-white"}`}
                            >
                              {type.label}
                            </p>
                            <p className="text-xs text-slate-400 mt-1">
            ...
            <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={amountRaised}
                      onChange={(e) =>
                        setAmountRaised(formatCurrency(e.target.value))
                      }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={pricePerShare}
                      onChange={(e) =>
                        setPricePerShare(e.target.value.replace(/[^0-9.]/g, ""))
                      }
                      onBlur={calculateShares}
                      placeholder="0.50"
                      className="w-full bg-slate-800 border border-slate-700 rounded-lg pl-7 pr-4 py-2 text-white placeholder-slate-500 focus:ring-2 focus:ring-gold-500 focus:border-transparent"
              ...
              <div>
                ...
                <input
                  ...
                  value={sharesIssued}
                  onChange={(e) =>
                    setSharesIssued(formatCurrency(e.target.value))
                  }
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Auto-calculated based on amount and price
                </p>
              </div>
                ...
                <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-1">
                        Price to exercise each warrant
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="1">
                        1 warrant per unit (full warrant)
                      </option>
              ...
              <div className="p-4 bg-gold-500/10 border border-gold-500/30 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div className="text-white">
                    {
                      FINANCING_TYPES.find((t) => t.value === financingType)
                        ?.label
                    }
                  </div>
                  ...
                  <div className="text-white">
                    ${pricePerShare || "---"} per{" "}
                    {unitType === "units" ? "unit" : "share"}
                  </div>
                  <div className="text-slate-400">
                    {unitType === "units" ? "Units" : "Shares"}:
                  </div>
                  ...
                  {hasWarrants && (
                    <>
                      ...
                      <div className="text-white">
                        ${warrantStrikePrice || "---"} strike, expires{" "}
                        {warrantExpiryDate || "---"}
                      </div>
                    </>
                  )}
          ...
          <div className="flex gap-3 mt-6">
            ...
            {step < 3 ? (
              ...
              <Button
                ...
                disabled={
                  creating || !amountRaised || !pricePerShare || !announcedDate
                }
              >
