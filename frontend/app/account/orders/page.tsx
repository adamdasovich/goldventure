...
export default function OrdersPage() {
  ...
  useEffect(() => {
    ...
    // Signed out: render the sign-in modal below rather than pushing at
    // /auth/login, which is a route this site does not have.
    if (!isAuthenticated) {
      setIsLoading(false);
      return;
    }
  ...
  // Signed out. The old condition sent this branch a spinner that never
  // resolved, which was invisible only because the redirect fired first.
  if (!isAuthenticated) {
    return (
      <SignInRequired
        title="Sign in to see your orders"
        destination="your order history"
      />
    );
  }
          ...
          <div key={order.id} className="glass rounded-xl overflow-hidden">
            ...
            <div className="bg-slate-800/50 px-6 py-4 border-b border-slate-700">
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <span
                  className={`px-3 py-1.5 text-xs font-medium rounded-full border ${statusColors[order.status] || statusColors.pending}`}
                >
            ...
            <div className="p-6">
              <div className="space-y-4">
                {order.items?.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-start gap-4 pb-4 border-b border-slate-700/50 last:border-0 last:pb-0"
                  >
                    {/* Item Icon */}
