...
export default function ClosedFinancingsClient() {
  const { user } = useAuth();
  ...
  const [editingFinancing, setEditingFinancing] =
    useState<ClosedFinancing | null>(null);
  ...
  const fetchClosedFinancings = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/closed-financings/?${params.toString()}`,
        {
          headers,
        },
      );
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load closed financings",
      );
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      const payload = {
        ...
        shares_issued: formData.shares_issued
          ? parseInt(formData.shares_issued)
          : null,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to ${editingFinancing ? "update" : "create"} financing`,
        );
      }

      const data = await response.json();
      alert(
        `Financing ${editingFinancing ? "updated" : "created"} successfully for ${data.financing?.company_name || "company"}`,
      );
      handleCloseAddModal();
      fetchClosedFinancings();
    } catch (err) {
    ...
    <div className="min-h-screen bg-slate-900">
      ...
      <section className="relative py-12 px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative max-w-7xl mx-auto text-center">
          ...
          <p className="text-sm text-slate-400 max-w-2xl mx-auto mb-6">
            New to mining financings? Read our guide on{" "}
            <Link
            ...
            — covers private placements, bought deals, flow-through shares,
            warrants, and the dilution math.
          </p>
          {user?.is_superuser && (
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-slate-400 mb-4">
                Showing {filteredFinancings.length} closed financing
                {filteredFinancings.length !== 1 ? "s" : ""}
              </p>
                        ...
                        <div className="mt-4 pt-4 border-t border-slate-700">
                          <div className="flex items-center gap-4 text-sm">
                            ...
                            {financing.warrant_strike_price && (
                              <span className="text-white">
                                Strike $
                                {Number(financing.warrant_strike_price).toFixed(
                                  3,
                                )}
                              </span>
                            )}
                            {financing.warrant_expiry_date && (
                              <span className="text-white">
                                Expires{" "}
                                {formatDate(financing.warrant_expiry_date)}
                              </span>
                            )}
                          </div>
                        </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-slate-800 rounded-lg p-6 max-w-2xl w-full my-8 max-h-[90dvh] overflow-y-auto">
