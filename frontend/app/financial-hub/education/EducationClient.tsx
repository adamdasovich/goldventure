...
interface EducationClientProps {
  /**
   * Modules fetched on the server so the first render carries them. Without
   * this the page showed a spinner to every crawler and every first-time
   * visitor while four published modules sat behind a fetch.
   */
  initialModules?: EducationalModule[];
}
...
export default function EducationHub({
  ...
  const [modules, setModules] = useState<EducationalModule[]>(initialModules);
  // Only spin when there is nothing to show. With server-supplied modules the
  // content is on screen immediately and the refetch happens underneath.
  ...
  const fetchModules = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api'}/education/modules/`, {
        headers: {
      ...
      <section className="relative py-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            <h1 className="font-display text-2xl sm:text-3xl md:text-4xl font-semibold mb-6 text-gold-400 animate-fade-in leading-tight tracking-tight italic">
              Educational Hub
            </h1>
