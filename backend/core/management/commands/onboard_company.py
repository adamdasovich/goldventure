    ...
    async def _process_company(self, url: str, options: dict):
        ...
        try:
            ...
            if company:
                ...
                # Queue RAG processing as background Celery tasks
                # This is non-blocking and uses Voyage AI for fast embeddings
                news_count = len(data.get('news', []))
                if news_count > 0:
                    @sync_to_async
                    def queue_rag_processing():
                        from core.tasks import process_company_news_for_rag_task, store_company_profile_in_rag_task
                        # Queue news processing task
                        news_task = process_company_news_for_rag_task.delay(company.id, limit=20)
                        # Queue profile storage task
                        profile_task = store_company_profile_in_rag_task.delay(company.id)
                        return news_task.id, profile_task.id

                    self.stdout.write(f"Queuing {news_count} news items for RAG processing (background task)...")
                    news_task_id, profile_task_id = await queue_rag_processing()
                    self.stdout.write(self.style.SUCCESS(
                        f"  Queued news RAG task: {news_task_id}"
                    ))
                    self.stdout.write(self.style.SUCCESS(
                        f"  Queued profile RAG task: {profile_task_id}"
                    ))
            else:
